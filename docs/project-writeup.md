...
We used Morph's EVM-compatible L2 for on-chain USDC settlement, viem for contract interaction, and Supabase Realtime to push the confirmation toast the moment the transaction is confirmed. Stripe handles card collection; Google Gemini generates invoices from a single sentence.
